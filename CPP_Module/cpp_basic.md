...
---
## 다형성
> 클래스를 만들고 상속을 할 때 오버라이딩을 하는데 이 때 문제가 발생할 수 있다.
c++ 컴파일러는 포인터 변수가 가리키고 있는 변수의 타입을 기준으로 함수를 호출하지 않고 포인터 타입을 기준으로 함수를 호출한다. <br>
```c++
int main()
{
	ClassA* ptr;
	ClassA aclass;
	ClassB bclass;	//ClassB는 ClassA를 상속하고 있다고 가정하자.
...
```
>위와 같이 상속관계에 있을 때 ptr을 이용해서 함수를 호출하면 ptr의 타입이
ClassA* 이기 때문에 ClassA의 show 함수만 두 번 호출하는 결과를 낳는다. <br>
이 문제를 해결하기 위해서 가상 함수를 사용한다. <br>

### 가상함수
> 가상 함수는 기본 클래스(상속되지 않은 클래스) 내에서 선언되어 파생 클래스에 의해 재정의되는 맴버함수이다. 포인터 또는 기본 클래스에 대한 참조(reference)를 사용하여 파생 클래스의 객체를 참조하면 해당 객체에 대해 가상 함수를 호출하고 파생 클래스의 함수를 실행할 수 있다. 이는 주로 실행시간 (Runtime)에 함수의 다형성을 구현하는데 사용된다.
- 규칙
1. 클래스의 public 섹션에 선언
2. 가상 함수는 static일 수 없고 다른 함수의 friend함수가 될 수 없다
3. 가상 함수는 실행시간 다형성을 얻기 위해 기본 클래스의 포인터 또는 참조를 통해 접근해야 한다.
4. 가상 함수의 프로토타입은 기본 클래스와 파생 클래스에서 동일하다.
5. 클래스는 가상 소멸자를 가질 수 있지만 가상 생성자는 갖지 못한다.

> 이렇게 가상함수를 이용하면 위 코드블록에서 부모 클래스 타입의 포인터인 ptr로 자식 클래스에서 오버라이딩된 함수를 호출할 수 있는 것이다.

### Reference: (https://yeolco.tistory.com/125) (https://velog.io/@underlier12/C-07-%EB%8B%A4%ED%98%95%EC%84%B1-dbk69cs1zz)
---
