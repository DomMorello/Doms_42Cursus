...
namespace ft
{
    ...
    // ############## list iterator ############################################################
    template<typename T>
    struct _list_iterator   //상속받아서 굳이 할 필요 없다.
    {
        ...
        typedef T*                                  pointer;
        typedef T&                                  reference;
        typedef std::bidirectional_iterator_tag     iterator_category;  //이터레이터의 성격을 보여주는 표지판
    ...
    // ########## Member types ##########
        ...
        typedef _list_iterator<T>                           iterator;   //직접 만들어야 함
        typedef _list_const_iterator<T>                     const_iterator; //직접 만들어야 함
    ...
    protected:
        ...
        _list_node_base     sentry_node;    //gnu와 다르게 따로 분리처리
        ...
        void _init()    //보초노드가 자기 자신을 가라킨다. 넥스트 프리브 전부다. 센트리노드가 항상 마지막에 위치/
        {
    ...
    // ########## (constructor) ##########
        ...
        //range (3)	
        template <class InputIterator>  //ex) list(int 10, int 2)로 하면 이 아래 생성자가 호출된다. 타입이 다르기 때문
    ...
    {
        _init();
        typedef typename ft::is_integer<InputIterator>::type is_integer_type;   //integer인지 확인해서 분기처리
